...
const stripePromise = loadStripe('pk_test_51Q68A0Rulbgvy4CQq3ylad4xFj583rKS8tMdCp9G0ZPZhXx1nCOu4NZAm8meE26r8jmsepxAyrZfJ3CvlhfmI6eQ000TtFRDBX');

const Cart = () => {
    const { cartItems, decreaseCartItemQuantity, addToCart, clearCart } = useCart();

    if (cartItems.length === 0) {
        return <div className='text-black text-3xl text-center my-72'>Cart is empty.</div>;
    }
  ...
  return (
    <div className='p-4 mt-16 max-w-[1400px] mx-auto'>
        ...
        <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-6'>
            {cartItems.map((item, index) => (
                <div key={index} className='bg-base-200 rounded-lg shadow-lg p-4 flex flex-col'>
                    ...
                    <p className='text-md mb-1'>Price: Rp.{(item.price).toFixed(2)}</p>
        ...
        <div className='text-center mt-8'>
            <p className='text-2xl font-semibold mb-4'>Total Price: Rp.{(totalPrice).toFixed(2)}</p>
            <button onClick={handleCheckout} className='btn btn-accent'>Checkout</button>
        </div>  
    </div>
  )
}
